...
var dims = function (i, el) {
  return {
    ...
    height: angular.element(el).height()
  }
}
    ...
    return function (scope, element) {
      var resize = function () {
          var parentRect = element.parent()[0].getBoundingClientRect(),
            prev = element.prev().length
              ? element.prev()[0].getBoundingClientRect().bottom
              : parentRect.top,
            next = element.next().length
              ? element.next()[0].getBoundingClientRect().top
              : parentRect.bottom
    ...
    return function (scope, element, attrs) {
      var selector = attrs.scrollBottomWithin,
        ...
        scrollBottom = function () {
          ...
          // TODO: externalize
          element
            .find('.scroll-instruction')
            .toggleClass('show-instruction', showScrollInstruction)
        },
        dimsToWatch = function () {
          return element
            .add(withinEl)
